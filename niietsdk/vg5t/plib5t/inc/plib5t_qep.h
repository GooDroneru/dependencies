...
#ifndef __PLIB5T_QEP_H
#define __PLIB5T_QEP_H
...
/**
  * @brief   Включение генерации запроса Rx DMA событию захвата
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void QEP_CAP_DMARxCmd(FunctionalState State)
{
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(QEP->DMAREQ_bit.DMARXEN, State);
}

/**
  * @brief   Включение генерации запроса Tx DMA событию захвата
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void QEP_CAP_DMATxCmd(FunctionalState State)
{
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(QEP->DMAREQ_bit.DMATXEN, State);
}
...
/**
  ...
  *                    Параметр принимает любую совокупность значений QEP_ITSource_x из @ref QEP_ITSource_define.
  * @param   State  Выбор состояния
  * @retval  void
  */
