...
#ifndef __PLIB5T_ADC_H
#define __PLIB5T_ADC_H
...
#define ADC_SEQ_Req_Total 8UL
#define ADC_DC_Total 4UL
#define ADC_CH_Total 5UL
...
/**
  * @brief  Выбор поддиапазона напряжения LDO АЦП
  */
typedef enum {
    ADC_AM_RangeLDO_0 = 0,   /*!< Поддиапазон от 0,81 В до 1,0 В */
    ADC_AM_RangeLDO_1 = 1,   /*!< Поддиапазон от 1,0 В до 1,32 В (Рекомендуется) */
} ADC_AM_RangeLDO_TypeDef;
#define IS_ADC_AM_RANGELDO(VALUE) (((VALUE) == ADC_AM_RangeLDO_0) ||  \
                                   ((VALUE) == ADC_AM_RangeLDO_1))

/**
  * @brief  Разрядность измерения АЦП
  */
typedef enum {
    ADC_AM_Resolution_6bit = ADC_ACTL_SELRES_6bit,   /*!< 6 бит */
    ADC_AM_Resolution_8bit = ADC_ACTL_SELRES_8bit,   /*!< 8 бит */
    ADC_AM_Resolution_10bit = ADC_ACTL_SELRES_10bit, /*!< 10 бит */
    ADC_AM_Resolution_12bit = ADC_ACTL_SELRES_12bit, /*!< 12 бит */
...
typedef enum {
    ...
    ADC_SEQ_StartEvent_TMR0 = ADC_EMUX_EM0_TMR0,         /*!< Сигнал от таймера TMR0 */
    ADC_SEQ_StartEvent_TMR1 = ADC_EMUX_EM0_TMR1,         /*!< Сигнал от таймера TMR1 */
    ADC_SEQ_StartEvent_TMR2 = ADC_EMUX_EM0_TMR2,         /*!< Сигнал от таймера TMR2 */
    ADC_SEQ_StartEvent_TMR3 = ADC_EMUX_EM0_TMR3,         /*!< Сигнал от таймера TMR3 */
    ADC_SEQ_StartEvent_PWM0A = ADC_EMUX_EM0_PWM0A,       /*!< Сигнал от PWM0 chanel A */
    ADC_SEQ_StartEvent_PWM0B = ADC_EMUX_EM0_PWM0B,       /*!< Сигнал от PWM0 chanel B */
    ADC_SEQ_StartEvent_PWM1A = ADC_EMUX_EM0_PWM1A,       /*!< Сигнал от PWM1 chanel A */
    ADC_SEQ_StartEvent_PWM1B = ADC_EMUX_EM0_PWM1B,       /*!< Сигнал от PWM1 chanel B */
    ADC_SEQ_StartEvent_PWM2A = ADC_EMUX_EM0_PWM2A,       /*!< Сигнал от PWM2 chanel A */
    ADC_SEQ_StartEvent_PWM2B = ADC_EMUX_EM0_PWM2B,       /*!< Сигнал от PWM2 chanel B */
...
#define IS_ADC_SEQ_START_EVENT(VALUE) (((VALUE) == ADC_SEQ_StartEvent_SwReq) || \
                                       ((VALUE) == ADC_SEQ_StartEvent_TMR32) || \
                                       ...
                                       ((VALUE) == ADC_SEQ_StartEvent_GPIOC) || \
...
/**
  ...
  */
typedef enum {
	ADC_SEQ_Average_Disable = ADC_SEQ_SRQCTL_QAVGVAL_Disable,    /*!< Усреднение выключено */
...
#define IS_ADC_SEQ_AVERAGE(VALUE) (((VALUE) == ADC_SEQ_Average_Disable) ||  \
								   ((VALUE) == ADC_SEQ_Average_2) ||  \
...
typedef enum {
    ...
    ADC_TSENS = 4, /*!< Канал 4 - канал измерения датчика TSENS */
...
/**
  * @brief   Включение LDO аналогового модуля АЦП
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void ADC_AM_LDOCmd(FunctionalState State)
{
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(ADC->ACTL_bit.ADCENLDO, State);
}

/**
  * @brief   Запуск аналогового модуля АЦП
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void ADC_AM_StartCmd(FunctionalState State)
{
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(ADC->ACTL_bit.ADCSTART, State);
}

/**
  * @brief   Отключение регуляторов уровня аналогового модуля АЦП
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void ADC_AM_DisableLVLCmd(FunctionalState State)
{
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(ADC->ACTL_bit.DISLVL, State);
}

/**
  * @brief   Настройка поддиапазона напряжения LDO аналогового модуля АЦП
  * @param   RangeLDO  Выбор поддиапазона напряжения
  * 					 (Рекомендуется работа в поддиапазоне 1)
  * @retval  void
  */
__STATIC_INLINE void ADC_AM_RangeLDOConfig(ADC_AM_RangeLDO_TypeDef RangeLDO)
{
    assert_param(IS_ADC_AM_RANGELDO(RangeLDO));

    WRITE_REG(ADC->ACTL_bit.RANGELDO, RangeLDO);
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void ADC_AM_ResolutionConfig(ADC_AM_Resolution_TypeDef Resolution)
{
    assert_param(IS_ADC_AM_RESOLUTION(Resolution));

    WRITE_REG(ADC->ACTL_bit.SELRES, Resolution);
}
...
/**
  ...
  */
__STATIC_INLINE uint32_t ADC_AM_GetResolution(void)
{
    assert_param(IS_ADC_AM_RESOLUTION(Resolution));

	return (uint32_t) READ_REG(ADC->ACTL_bit.SELRES);
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void ADC_AM_CalibCmd(FunctionalState State)
{
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(ADC->ACTL_bit.CALEN, State);
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void ADC_AM_CalibStartCmd(FunctionalState State)
{
    assert_param(IS_FUNCTIONAL_STATE(State));

    WRITE_REG(ADC->ACTL_bit.CALSTART, State);
}
...
/**
  ...
  */
__STATIC_INLINE FunctionalState ADC_AM_CalibStatus(void)
{
    return (FunctionalState) READ_BIT(ADC->ACTL, ADC_ACTL_CALBUSY_Msk);
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void ADC_AM_SetCalibVal(uint32_t Val)
{
    WRITE_REG(ADC->ACTL_bit.CALIN, Val);
    WRITE_REG(ADC->ACTL_bit.CALLOAD, 1);
}
...
/**
  ...
  * @retval  Val  Значение. Диапазон значений -64…63, величина в
                  дополнительном коде.
  */
__STATIC_INLINE uint32_t ADC_AM_GetCalibVal()
{
    return READ_REG(ADC->ACTL_bit.CALOUT);
}
...
/**
  ...
  */
__STATIC_INLINE FlagStatus ADC_AM_ReadyStatus()
{
...
/**
  ...
  */
__STATIC_INLINE FlagStatus ADC_AM_BusyStatus()
{
    return (FlagStatus)READ_BIT(ADC->BSTAT, ADC_BSTAT_ADCBUSY_Msk);
}
...
/**
  * @brief   Настройка задержки канала АЦП
  * @param   Channel_Num  Выбор канала
  * @param   Delay  Значение задержки
  * @retval  void
  */
__STATIC_INLINE void ADC_CH_DelayConfig(ADC_CH_Num_TypeDef Channel_Num, uint16_t Delay)
{
    assert_param(IS_ADC_CH_NUM(Channel_Num));

    WRITE_REG(ADC->CHDELAY[Channel_Num].CHDELAY, Delay);
}
