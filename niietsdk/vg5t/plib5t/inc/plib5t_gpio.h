...
#ifndef __PLIB5T_GPIO_H
#define __PLIB5T_GPIO_H
...
/**
  * @brief   Разрешение генерации Rx запросов к DMA
  * @param   GPIOx  выбор порта, где x=A|B
  * @param   Pin  Выбор пинов. Любая совокупность значений значений GPIO_Pin_x (@ref GPIO_Pin_Define).
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void GPIO_DMARxReqCmd(GPIO_TypeDef* GPIOx, uint32_t Pin, FunctionalState State)
{
    assert_param(IS_GPIO_PERIPH(GPIOx));
    assert_param(IS_GPIO_PIN(Pin));
    assert_param(IS_FUNCTIONAL_STATE(State));
    ...
    if (State == ENABLE)
        WRITE_REG(GPIOx->DMARXREQSET, Pin);
    else
        WRITE_REG(GPIOx->DMARXREQCLR, Pin);
}

/**
  * @brief   Разрешение генерации Tx запросов к DMA
  * @param   GPIOx  выбор порта, где x=A|B
  * @param   Pin  Выбор пинов. Любая совокупность значений значений GPIO_Pin_x (@ref GPIO_Pin_Define).
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void GPIO_DMATxReqCmd(GPIO_TypeDef* GPIOx, uint32_t Pin, FunctionalState State)
{
    assert_param(IS_GPIO_PERIPH(GPIOx));
    assert_param(IS_GPIO_PIN(Pin));
    assert_param(IS_FUNCTIONAL_STATE(State));
    ...
    if (State == ENABLE)
        WRITE_REG(GPIOx->DMATXREQSET, Pin);
    else
        WRITE_REG(GPIOx->DMATXREQCLR, Pin);
}
