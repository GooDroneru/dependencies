...
#ifndef __PLIB5T_PWM_H
#define __PLIB5T_PWM_H
...
/** @defgroup PWM_CNTEN_Define Управление счетчиками таймеров блоков ШИМ
  * @{
  */

#define PWM_CNTEN_0 SIU_CNTEN_PWM0EN_Msk /*!< Счетчик блока ШИМ0 */
#define PWM_CNTEN_1 SIU_CNTEN_PWM1EN_Msk /*!< Счетчик блока ШИМ1 */
#define PWM_CNTEN_2 SIU_CNTEN_PWM2EN_Msk /*!< Счетчик блока ШИМ2 */
#define PWM_CNTEN_All  (PWM_CNTEN_0 | \
						PWM_CNTEN_1 | \
						PWM_CNTEN_2) /*!< Все блоки выбраны */

#define IS_PWM_CNTEN(VALUE) (((VALUE) & ~PWM_CNTEN_All) == 0)
...
/**
  ...
  *                 Параметр принимает любою совокупность значений из @ref PWM_CNTEN_Define.
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void PWM_TB_PrescCmd(uint32_t Counter, FunctionalState State)
{
    assert_param(IS_PWM_CNTEN(Counter));
    assert_param(IS_FUNCTIONAL_STATE(State));

    MODIFY_REG(SIU->CNTEN, Counter, State ? Counter : 0);
}
