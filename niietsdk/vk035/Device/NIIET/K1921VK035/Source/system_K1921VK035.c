...
void ClkInit()
{    
    // --- НАЧАЛО: Анти-паразитный костыль ---
    // 1. Принудительно выключаем питание PLL (режим Powerdown)
    // Даже если PMU выключен, попытка записи может переключить триггеры.
    PMU->PDEN_bit.PLLPD = 1; // Убеждаемся, что он в пауэрдауне
    for (volatile int i = 0; i < 1000; i++); // Небольшая задержка

    // 2. Полностью обнуляем регистр конфигурации PLL.
    // Это критически важно! Если там висит мусор, LOCK никогда не взведется.
    RCU->PLLCFG = 0;

    // 3. Отключаем выходную частоту и bypass, если они были включены
    RCU->PLLCFG_bit.BYPASS = 0;
    RCU->PLLCFG_bit.OUTEN = 0;

    // 4. Убеждаемся, что системный клок сидит на надежном внутреннем источнике.
    // Принудительно переключаем систему на OSICLK, игнорируя ошибки.
    RCU->SYSCLKCFG = (RCU_SYSCLKCFG_SYSSEL_OSICLK << RCU_SYSCLKCFG_SYSSEL_Pos);
    for (volatile int i = 0; i < 100; i++); // Ждем переключения

    // 5. Сбрасываем флаги статуса (если висят ошибки)
    // Записываем 1 во все флаги в регистре INTSTAT для их сброса.
    RCU->INTSTAT = 0x000FFFFF; // Сброс всех флагов прерываний RCU
    // --- КОНЕЦ: Анти-паразитный костыль ---
