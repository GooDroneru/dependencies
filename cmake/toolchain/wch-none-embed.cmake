...
if(_PATH_GCC)
    ...
    if(NOT EXISTS "${_GCC_EXE}")
        # Broken symlink on PATH (e.g. /usr/local/bin/riscv-none-embed-gcc ->
        # /opt/... that was never installed): find_program returns the dangling
        # target. Ignore it and fall back to the pinned/devcontainer root below.
        message(STATUS "Ignoring non-existent RISC-V GCC on PATH: ${_GCC_EXE}")
        unset(_GCC_EXE)
    else()
        get_filename_component(_BIN_DIR "${_GCC_EXE}" DIRECTORY)
        # Detect layout: if bin dir contains "bin" in name -> standard layout (toolchain_root = parent)
        # If gcc is directly in toolchain root -> non-standard (toolchain_root = bin_dir)
...
if(NOT DEFINED _GCC_EXE)
    if(DEFINED ENV{RISCV_TOOLCHAIN_DIR} AND NOT "$ENV{RISCV_TOOLCHAIN_DIR}" STREQUAL "")
        ...
        if(EXISTS "${_DC_TOOLCHAIN_ROOT}/bin/riscv-none-embed-gcc")
            set(_TOOLCHAIN_ROOT "${_DC_TOOLCHAIN_ROOT}")
            message(STATUS "Using devcontainer toolchain: ${_TOOLCHAIN_ROOT}")
        else()
            set(_TOOLCHAIN_ROOT "${_CI_TOOLCHAIN_ROOT}")
